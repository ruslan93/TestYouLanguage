...
- (void)drawText:(CGSize)rectSize context:(CGContextRef)c
{
  ...
  NSDictionary* valueFontAttributes = @{NSFontAttributeName: [UIFont systemFontOfSize:valueFontSize weight:UIFontWeightRegular], NSForegroundColorAttributeName: [UIColor whiteColor], NSParagraphStyleAttributeName: textStyle};
  ...
  // set the decimal font size
    ...
    if (decimalLocation != NSNotFound){
        NSDictionary* valueDecimalFontAttributes = @{NSFontAttributeName: [UIFont fontWithName: self.valueFontName size:self.valueDecimalFontSize == -1 ? valueFontSize : self.valueDecimalFontSize], NSForegroundColorAttributeName: [UIColor whiteColor], NSParagraphStyleAttributeName: textStyle};
    ...
    // add ball text
    NSMutableAttributedString *ballText = [NSMutableAttributedString new];
    NSDictionary* valueFontAttributesClass = @{NSFontAttributeName: [UIFont systemFontOfSize:rectSize.height/10], NSForegroundColorAttributeName:[UIColor whiteColor], NSParagraphStyleAttributeName: textStyle};
    CGFloat k = (100.0 / self.totalBall);
    NSString *stringValue = [NSString stringWithFormat:@"%ld / %ld", (long)(self.value / k), (long)self.totalBall];
    NSAttributedString* valueBall = [[NSAttributedString alloc] initWithString:stringValue attributes:valueFontAttributesClass];
    [ballText appendAttributedString:valueBall];
    
    [ballText drawAtPoint: CGPointMake(
                                       rectSize.width/2 - ballText.size.width/2,
                                       rectSize.height/2 + percentSize.height/1.5
                                       )];
